####
#### CER eDNA study
#### No.4 CMR-002 Eukaryote: Standard check
####

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder04 <- "04_STDCheck_EukOut"
...
load("01_CERrice2017_EukSeqOut/01_CERrice2017_EukSeqOut.RData")
tax_claident <- read.csv("03_TaxaSTDcombine_EukOut/claident_tax_revise.csv")
sample_sheet <- read.csv("../sampledata_all/20181203_CMR-002_MS7290407-300V2.csv")

# Set parameters
std_copy_n <- c(10000,5000,2500,1250,250)
col_name_level <- "Family"
seq_summary_fig_file <- sprintf("%s/Seq_Summary.pdf", output_folder04)
std_seq_head <- "STD_Euk"
...
all(sample_sheet$Sample_Name2 == rownames(seqtab_nochim)) # Should be TRUE
nrow(sample_sheet) == nrow(seqtab_nochim) # For CMR-002, FALSE
...
g4 <- g4 + ggtitle("CMR-002, CER rice Eukaryote seqs")
...
## Temporal workspace
save.image(sprintf("%s/04_STDCheckNotConv_EukOut.RData", output_folder04))
...
#     file = sprintf("%s/04_STDCheck_EukOut.RData", output_folder04))
save.image(sprintf("%s/04_STDCheck_EukOut.RData", output_folder04))

#### save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/04_SessionInfo_EukSTDcheckConv_%s.txt", substr(Sys.time(), 1, 10)))
