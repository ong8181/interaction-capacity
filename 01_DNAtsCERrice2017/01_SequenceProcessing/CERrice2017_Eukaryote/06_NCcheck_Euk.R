####
#### CER eDNA study
#### No.6 CMR-002 Eukaryote: Negative & Positive control check
####

# Load workspace and data
load("05_ReadNSummary_EukOut/05_ReadNSummary_EukOut.RData")

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder06 <- "06_NCcheck_EukOut"
...
p14 <- p14 + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + ylim(0,30000)
...
p18 <- p18 + geom_bar(stat = "identity", colour = NA) + theme(axis.text.x = element_text(angle = 90)) + scale_fill_igv() + ylim(0,40000)
...
#     file = sprintf("%s/06_NCcheck_EukOut.RData", output_folder06))
save.image(sprintf("%s/06_NCcheck_EukOut.RData", output_folder06))
