####
#### CER rice 2017 eDNA analysis
#### No.3 STD sequences assignment
####

# Create output directory
dir.create("03_TaxaSTDcombine_FungiOut")

# Load Claident taxa assignment
claident_tax <- read.delim("01_CERrice2017_FungiSeqOut/FungiASV_merge_classigntax")
head(claident_tax)

# Load Blastn taxa assignment for STD sequences
# Select sequences with < 2 mismatches and length > 220
blastn_std0 <- read.table("02_ident_FungiSTD_BLASTnOut/FungiSTD_out.txt")
...
write.csv(claident_tax, "03_TaxaSTDcombine_FungiOut/claident_tax_revise.csv", row.names = F)

#### save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/03_SessionInfo_FungiSTDident_%s.txt", substr(Sys.time(), 1, 10)))
