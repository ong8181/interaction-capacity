####
#### CER eDNA study
#### No.4 RMR-078 Fungi: Standard check
####

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder04 <- "04_STDCheck_FungiOut"
...
load("01_CERrice2017_FungiSeqOut/01_CERrice2017_FungiSeqOut.RData")
tax_claident <- read.csv("03_TaxaSTDcombine_FungiOut/claident_tax_revise.csv")
sample_sheet <- read.csv("../sampledata_all/20181203_RMR-078_MS6435543-600V3.csv")

# Set parameters
std_copy_n <- c(400,200,100,40,20)
...
all(sample_sheet$Sample_Name2 == rownames(seqtab_nochim)) # For RMR-078, FALSE
nrow(sample_sheet) == nrow(seqtab_nochim) # For RMR-078, FALSE
...
g4 <- g4 + ggtitle("RMR-078, CER rice Fungi seqs")
...
## Temporal workspace
save.image(sprintf("%s/04_STDCheckNotConv_FungiOut.RData", output_folder04))
...
#     file = sprintf("%s/04_STDCheck_FungiOut.RData", output_folder04))
save.image(sprintf("%s/04_STDCheck_FungiOut.RData", output_folder04))

#### save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/04_SessionInfo_FungiSTDcheckConv_%s.txt", substr(Sys.time(), 1, 10)))
