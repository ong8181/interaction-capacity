####
#### CER eDNA study
#### No.5 RMR-078 Fungi: Read number visualization
####

# Load workspace and data
load("04_STDCheck_FungiOut/04_STDCheck_FungiOut.RData")

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder05 <- "05_ReadNSummary_FungiOut"
...
write.csv(sample_summary, sprintf("%s/summary_sheet_Fungi.csv", output_folder05))

# Repalce sample_sheet
sample_sheet <- sample_summary

# Save important objects
saveRDS(sample_sheet, sprintf("%s/sample_sheet_Fungi.obj", output_folder05))
saveRDS(seqtab_nochim, sprintf("%s/seqtab_nochim_Fungi.obj", output_folder05))
saveRDS(seqtab_conv, sprintf("%s/seqtab_conv_Fungi.obj", output_folder05))
saveRDS(new_std_table, sprintf("%s/new_std_table_Fungi.obj", output_folder05))
saveRDS(tax_claident, sprintf("%s/tax_claident_Fungi.obj", output_folder05))
saveRDS(tax_claident2, sprintf("%s/tax_claident2_Fungi.obj", output_folder05))
saveRDS(taxa_w_std, sprintf("%s/taxa_w_std_Fungi.obj", output_folder05))
saveRDS(taxa_wo_std, sprintf("%s/taxa_wo_std_Fungi.obj", output_folder05))

# Save and output results
#save(list = ls(all.names = TRUE),
#     file = sprintf("%s/05_ReadNSummary_FungiOut.RData", output_folder05))
save.image(sprintf("%s/05_ReadNSummary_FungiOut.RData", output_folder05))
