####
#### CER eDNA study
#### No.4 RMR-099 Invertebrate: Standard check
####

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder04 <- "04_STDCheck_InvOut"
...
load("01_CERrice2017_InvSeqOut/01_CERrice2017_InvSeqOut.RData")
tax_claident <- read.csv("03_TaxaSTDcombine_InvOut/claident_tax_revise.csv")
sample_sheet <- read.csv("../sampledata_all/20181203_RMR-099_MS6988275-500V2.csv")

# Set parameters
std_copy_n <- c(200,100,50,25,5)
col_name_level <- "Family"
seq_summary_fig_file <- sprintf("%s/Seq_Summary.pdf", output_folder04)
...
all(sample_sheet$Sample_Name2 == rownames(seqtab_nochim)) # For RMR-099, FALSE
nrow(sample_sheet) == nrow(seqtab_nochim) # For RMR-099, FALSE
...
new_std_table <- data.frame(std_rank1 = MergeSTD(detected_std_name[1], std_data = std_table),
                            ...
                            std_rank3 = MergeSTD(detected_std_name[4], std_data = std_table),
                            std_rank4 = MergeSTD(detected_std_name[3], std_data = std_table),
...
g1 <- g1 + geom_histogram() + geom_hline(yintercept = 0, linetype = 2)
...
g3 <- g3 + geom_point(size = 3) + scale_color_manual(name = "Regression slope", values = c("red3", "darkred", "black"))
g3 <- g3 + geom_abline(intercept = 0, slope = c(max(coef_summary), median(coef_summary), min(coef_summary)), color = c("red3", "darkred", "black"))
...
g4 <- PlotStyle(g4) + theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 6))
# Overwrite standard DNA reads
g4 <- g4 + geom_segment(data = std_table_plot, aes(x = sample, xend = sample, y = min, yend = max),
                        colour = "red3", size = 1.5, alpha = 0.5)
g4 <- g4 + ggtitle("RMR-099, CER rice Invertebrate seqs")
...
## Temporal workspace
save.image(sprintf("%s/04_STDCheckNotConv_InvOut.RData", output_folder04))
...
write.csv(new_seqtab_print, sprintf("%s/allseq.csv", output_folder04))
write.csv(new_std_table, sprintf("%s/stdseq.csv", output_folder04))
write.csv(tax_claident2, sprintf("%s/tax.csv", output_folder04))
...
# Exclude consecutive invalid indices
seqtab_conv_w_std0[invalid_index[1:4],] <- (seqtab_conv_w_std0[before_invalid[1:4],] + seqtab_conv_w_std0[after_invalid[1:4],])/2
# Interpolate consecutive invalid indices
seqtab_conv_w_std0[invalid_index[5],] <- (2*seqtab_conv_w_std0[before_invalid[5],] + seqtab_conv_w_std0[after_invalid[6],])/3
seqtab_conv_w_std0[invalid_index[6],] <- (seqtab_conv_w_std0[before_invalid[5],] + 2*seqtab_conv_w_std0[after_invalid[6],])/3
...
#     file = sprintf("%s/04_STDCheck_InvOut.RData", output_folder04))
save.image(sprintf("%s/04_STDCheck_InvOut.RData", output_folder04))

#### save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/04_SessionInfo_InvSTDcheckConv_%s.txt", substr(Sys.time(), 1, 10)))
