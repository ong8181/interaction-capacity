...
dir.create("03_TaxaSTDcombine_ProkOut")

# Load Claident taxa assignment
claident_tax <- read.delim("01_CERrice2017_ProkSeqOut/ProkASV_merge_classigntax")
head(claident_tax)
...
write.csv(claident_tax, "03_TaxaSTDcombine_ProkOut/claident_tax_revise.csv", row.names = F)

#### save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/03_SessionInfo_ProkSTDident_%s.txt", substr(Sys.time(), 1, 10)))
