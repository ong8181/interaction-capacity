####
#### CER eDNA study
#### No.5 RMR-076 Prokaryote: Read number visualization
####

# Load workspace and data
load("04_STDCheck_ProkOut/04_STDCheck_ProkOut.RData")

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder05 <- "05_ReadNSummary_ProkOut"
...
write.csv(sample_summary, sprintf("%s/summary_sheet_Prok.csv", output_folder05))

# Repalce sample_sheet
sample_sheet <- sample_summary

# Save important objects
saveRDS(sample_sheet, sprintf("%s/sample_sheet_Prok.obj", output_folder05))
saveRDS(seqtab_nochim, sprintf("%s/seqtab_nochim_Prok.obj", output_folder05))
saveRDS(seqtab_conv, sprintf("%s/seqtab_conv_Prok.obj", output_folder05))
saveRDS(new_std_table, sprintf("%s/new_std_table_Prok.obj", output_folder05))
saveRDS(tax_claident, sprintf("%s/tax_claident_Prok.obj", output_folder05))
saveRDS(tax_claident2, sprintf("%s/tax_claident2_Prok.obj", output_folder05))
saveRDS(taxa_w_std, sprintf("%s/taxa_w_std_Prok.obj", output_folder05))
saveRDS(taxa_wo_std, sprintf("%s/taxa_wo_std_Prok.obj", output_folder05))

# Save and output results
#save(list = ls(all.names = TRUE),
#     file = sprintf("%s/05_ReadNSummary_ProkOut.RData", output_folder05))
save.image(sprintf("%s/05_ReadNSummary_ProkOut.RData", output_folder05))
