####
#### CER eDNA study
#### No.6 RMR-076 Prokaryote: Negative & Positive control check
####

# Load workspace and data
load("05_ReadNSummary_ProkOut/05_ReadNSummary_ProkOut.RData")

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder06 <- "06_NCcheck_ProkOut"
...
p11 <- p10 + xlim(as.Date(c("2017-05-23", "2017-06-07"))) + ylim(0,20000)
p12 <- p10 + xlim(as.Date(c("2017-06-08", "2017-09-23"))) + ylim(0,20000)
...
p13 <- p13 + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + ylim(0,15000)
...
p14 <- p14 + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + ylim(0,20000)
...
#     file = sprintf("%s/06_NCcheck_ProkOut.RData", output_folder06))
save.image(sprintf("%s/06_NCcheck_ProkOut.RData", output_folder06))
