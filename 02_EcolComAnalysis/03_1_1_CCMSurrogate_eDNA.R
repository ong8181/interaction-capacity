...
cycle_set <- 1:floor(nrow(causal_dnaxdna)/3)
N_SURR <- 1000
E_RANGE <- 1:14
cycle_n <- 1
...
for(data_set_id in 1){
  ...
  #for(i in 1:nrow(causal_data)){
    ...
    if(any(cycle_n == seq(30000,total_cycle,30000))) saveRDS(ednacom_sur_res, sprintf("%s/tmp%s.obj", output_folder03, cycle_n))
  }
...
save(list = ls(all.names = TRUE),
     file = sprintf("%s/%s_1_1_eDNA.RData", output_folder03, output_folder03))
#save.image(sprintf("%s/%s_1_1_eDNA.RData", output_folder03, output_folder03))

# Save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/%s_1_1_eDNA_SessionInfo_%s.txt", output_folder03, substr(Sys.time(), 1, 10)))
