...
cycle_set <- (floor(nrow(causal_dnaxdna)/3)+1):(floor(nrow(causal_dnaxdna)/3)*2)
N_SURR <- 1000
E_RANGE <- 1:14
cycle_n <- 1
...
for(data_set_id in 1){
  ...
  if(data_set_id == 1){
    ...
    saveRDS(ednacom_sur_res1, sprintf("%s/ednacom_sur_res1_2.obj", output_folder03))
  }else if(data_set_id == 2){
...
save(list = ls(all.names = TRUE),
     file = sprintf("%s/%s_1_2_eDNA.RData", output_folder03, output_folder03))

# Save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/%s_1_2_eDNA_SessionInfo_%s.txt", output_folder03, substr(Sys.time(), 1, 10)))
