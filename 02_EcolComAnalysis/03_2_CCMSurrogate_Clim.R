...
#### No. 3.2 Perform suggogate analysis for potentially causal variables
####
...
# Set parameters
total_cycle <- nrow(causal_climdna) + nrow(causal_dnaclim)
N_SURR <- 1000
E_RANGE <- 1:14
cycle_n <- 1
...
for(data_set_id in 2:3){
  ...
  for(i in 1:nrow(causal_data)){
    ...
    #if(any(cycle_n == seq(50000,total_cycle,50000))) saveRDS(ednacom_sur_res, sprintf("%s/tmp%s.obj", output_folder03, cycle_n))
  }
  
  # Bind results
  if(data_set_id == 1){
    ednacom_sur_res1 <- ednacom_sur_res
    saveRDS(ednacom_sur_res1, sprintf("%s/ednacom_sur_res1.obj", output_folder03))
  }else if(data_set_id == 2){
...
# Bind surrogate resutls
#causal_dnaxdna <- bind_cols(causal_dnaxdna, ednacom_sur_res1)
causal_climdna <- bind_cols(causal_climdna, ednacom_sur_res2)
causal_dnaclim <- bind_cols(causal_dnaclim, ednacom_sur_res3)
#saveRDS(causal_dnaxdna, sprintf("%s/causal_dnaxdna.obj", output_folder03))
saveRDS(causal_climdna, sprintf("%s/causal_climdna.obj", output_folder03))
saveRDS(causal_dnaclim, sprintf("%s/causal_dnaclim.obj", output_folder03))

# Save workspace and ojcects
save(list = ls(all.names = TRUE),
     file = sprintf("%s/%s_2_Clim.RData", output_folder03, output_folder03))

# Save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/%s_2_Clim_SessionInfo_%s.txt", output_folder03, substr(Sys.time(), 1, 10)))
