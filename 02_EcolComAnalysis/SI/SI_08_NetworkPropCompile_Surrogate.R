...
#### SI: SurrogateTest: No.8 Compile network properties
####

# Load workspace and objects
load("SI_07_RegularizedSmap_SurrogateOut/SI_07_RegularizedSmap_SurrogateOut.RData")

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder08 <- "SI_08_NetworkPropCompile_SurrogateOut"
dir.create(output_folder08)

# Load library
library(Rcpp); packageVersion("Rcpp") # 1.0.3, 2020.1.29
library(RcppArmadillo); packageVersion("RcppArmadillo") # 0.9.800.3.0, 2020.1.29
library(Matrix); packageVersion("Matrix") # 1.2.18, 2020.1.29
...
edna_prop$total_dna <- rowSums(edna_surr, na.rm = T)
edna_prop$total_div <- rowSums(edna_surr > 0, na.rm = T)
...
# Add C.V. of community dynamics (calculate CV by sliding window approach)
...
saveRDS(edna_prop, sprintf("%s/edna_prop.obj", output_folder08))
...
writeLines(capture.output(sessionInfo()),
           sprintf("SI_00_SessionInfo_Surrogate/%s_SessionInfo_%s.txt", output_folder08, substr(Sys.time(), 1, 10)))
