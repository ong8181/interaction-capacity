...
#### Single varialbe regularrized S-map to check statistical artifacts
####

#-------------------- Determine best theta --------------------#
det_bestPar_reglSmap_surrogate <- function(taxa_list,
  ...
  valid_id_dna <- !is.na(match(causal_dnasurr$effect_var, taxa_list)) & !is.na(match(causal_dnasurr$causal_var, taxa_list))
  valid_id_clm <- !is.na(match(causal_dnaclimsurr$effect_var, taxa_list))
  causal_dnaxdna_sub <- causal_dnasurr[valid_id_dna,]
  causal_dnaclim_sub <- causal_dnaclimsurr[valid_id_clm,]
  ...
  best_par_block <- pforeach(i = 1:length(taxa_list), .c=bind_rows, .seed = taxa_random_seed)({
    # Set time
    start_time <- proc.time()[3]
    
    # Extract information for S-map
    E_tax <- Eedna_surr[taxa_list[i]]
    
    effect_tax <- taxa_list[i]
    causal_tax <- as.character(causal_dnaxdna_sub[causal_dnaxdna_sub$effect_var == effect_tax,]$causal_var)
    causal_clim <- as.character(causal_dnaclim_sub[causal_dnaclim_sub$effect_var == effect_tax,]$cause_var)
    ...
    if(embedding == "naive"){
      ...
      if(E_tax == 1 & n_cause == 0){
        block_smap <- matrix(as.numeric(scale(edna_surr[,c(effect_tax)])), ncol = 1)
      }else{
        # Include time-lag components if the number of causal variables is smaller than E
        block_smap <- apply(cbind(edna_surr[,c(effect_tax, causal_tax)],
                                  clim_all[,c(causal_clim)]), 2, function(x) as.numeric(scale(x)))
        if(E_tax != 1){
          block_lags <- make_block(as.numeric(scale(edna_surr[,effect_tax])), max_lag = E_tax)
      ...
      if(E_tax == 1 & n_cause == 0){
        ...
        # Include time-lag components if the number of causal variables is smaller than E
        block_lags <- make_block(as.numeric(scale(edna_surr[,effect_tax])), max_lag = E_tax - n_cause)
        block_smap <- apply(cbind(edna_surr[,c(effect_tax, causal_tax)],
                                  clim_all[,c(causal_clim)]), 2, function(x) as.numeric(scale(x)))
      ...
      if(E_tax == 1){
        block_smap <- as.numeric(scale(edna_surr[,effect_tax]))
      }else if(n_cause+1 >= E_tax){
        ...
        block_smap1 <- cbind(edna_surr, clim_all)[,c(effect_tax, causal_order)]
        block_smap <- apply(block_smap1, 2, function(x) as.numeric(scale(x)))
      }else{
        # Include time-lag components if the number of causal variables is smaller than E
        block_lags <- make_block(as.numeric(scale(edna_surr[,effect_tax])), max_lag = E_tax-n_cause)
        block_smap1 <- apply(cbind(edna_surr[,c(effect_tax, causal_tax)],
                                   clim_all[,c(causal_clim)]), 2, function(x) as.numeric(scale(x)))
    ...
    if(length(causal_tax) == 0) causal_tax <- NA
    
    # Store results
    if(all(is.na(best_par_check[,3]))){
      best_par <- data.frame(theta = NA, lambda = NA)
      best_par <- as_tibble(bind_cols(best_par, tibble(causal_tax = causal_tax) %>% nest(data = everything())))
    }else{
...
multi_reglSmap_all_surrogate <- function(taxa_list,
                                      bestPar_block,
                                      lib = edna_lib,
                                      pred = lib,
                                      embedding = "naive", # "intermediate", "limited_by_E"
  ...
  smap_res <- pforeach(i = 1:length(taxa_list), .combine = c)({
    ...
    effect_tax <- taxa_list[i]
    causal_tax <- unnest(bestPar_block[i,], cols = 1:3)$causal_tax
    if(length(causal_tax) == 1) if(is.na(causal_tax)) causal_tax <- character(0)
    causal_clim <- as.character(causal_dnaclim_sub[causal_dnaclim_sub$effect_var == effect_tax,]$cause_var)
    ...
    if(!is.na(bestPar_block[i,1])){
      smap_res_tmp  <- try(extended_lnlp(block_smap, lib = lib, pred = pred,
                                     tp = 1, target_column = 1,
                                     theta = as.numeric(bestPar_block[i,"theta"]),
                                     method = "s-map",
                                     regularized = regularized,
                                     lambda = as.numeric(bestPar_block[i,"lambda"]),
                                     alpha = alpha, # Ridge S-map
